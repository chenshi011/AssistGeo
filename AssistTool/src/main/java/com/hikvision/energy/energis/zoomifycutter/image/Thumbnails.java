...
public class Thumbnails {
	 ...
	 * @throws IOException
	 */
	public static String generate(ImageFile imageFile, String outPut, String fileName) throws IOException {
		return generate(imageFile, outPut, fileName, DEFAULT_THUMBNAILS_SIZE, DEFAULT_THUMBNAILS_SIZE,
				DEFAULT_KEEY_ASPECT_RATIO);
	}
	...
	public static String generate(ImageFile imageFile, String outPut, String fileName, int width, int height,
		...
		} else if (!fileName.toLowerCase().endsWith("." + THUMBNAILS_TYPE)) {
			fileName += "." + THUMBNAILS_TYPE;
		}
		...
		if (imgW <= width && imgH <= height) {
			...
			graphics2d.dispose();
			// 保存文件
			ImageIO.write(destination, "jpg", thumbnailsFile);
			// 释放资源
			destination.flush();
			source.flush();
			imageReader.dispose();
	...
	public static void main(String[] args) throws IOException {
		generate(new ImageFile(new File("D:\\test\\map\\heren.png")), "D:\\test\\out", "test", 128, 128, false);
	}
}
